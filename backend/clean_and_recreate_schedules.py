#!/usr/bin/env python3
"""
Script to clean old schedules and recreate with correct enum values
"""
import sys
import os
...
def clean_and_recreate_schedules():
    """Clean old schedules and recreate with correct enum values"""
    
    with get_db() as session:
        try:
            print("=== Cleaning and Recreating Schedules ===")
            ...
            # 2. Delete all existing notifications for this user
            existing_notifications = session.query(Notification).filter(
                Notification.user_id == elderly_user.id
            ).all()
            
            if existing_notifications:
                print(f"🗑️ Deleting {len(existing_notifications)} existing notifications...")
                for notif in existing_notifications:
                    session.delete(notif)
                session.commit()
                print("✅ All old notifications deleted")
            else:
                print("✅ No existing notifications to delete")
            
            # 3. Create new schedules with correct enum values
            ...
            # Commit all changes
            session.commit()
            print("\n🎉 All schedules recreated successfully!")
            ...
            session.rollback()
            print(f"❌ Error recreating schedules: {e}")
            import traceback
            traceback.print_exc()
            return False

if __name__ == "__main__":
    success = clean_and_recreate_schedules()
    if success:
        print("\n✅ Schedule recreation completed successfully!")
        print("You can now test the family connection feature in the Android app.")
    else:
        print("\n❌ Failed to recreate schedules.")
        sys.exit(1) 